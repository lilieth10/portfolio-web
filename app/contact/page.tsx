  ...
  const ContactInfo = ({ icon: Icon, title, content, link }: { icon: any; title: string; content: string; link?: string }) => (
    ...
    >
      <Card className="bg-white/90 dark:bg-black/50 backdrop-blur-lg border border-gray-200 dark:border-gray-800 p-6 hover:border-purple-500 hover:shadow-lg hover:shadow-purple-500/20 transition-all duration-300">
        ...
        {link ? (
          <a href={link} target="_blank" rel="noopener noreferrer" className="text-gray-700 dark:text-gray-400 hover:text-purple-400 transition-colors duration-300">
            {content}
          </a>
        ) : (
          ...
          >
            <Card className="bg-white/90 dark:bg-black/50 backdrop-blur-lg border border-gray-200 dark:border-gray-800 p-8">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-gray-900 dark:text-white mb-2">Nombre</label>
                  <input
                    type="text"
                    name="name"
                    required
                    placeholder="Tu nombre"
                    className="w-full px-4 py-3 bg-white/50 dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition-colors duration-300"
                  />
                </div>

                <div>
                  <label className="block text-gray-900 dark:text-white mb-2">Correo Electrónico</label>
                  <input
                    type="email"
                    name="email"
                    required
                ...
                <div>
                  <label className="block text-gray-900 dark:text-white mb-2">Mensaje</label>
                  <textarea
                    name="message"
                    required
                    placeholder="Escribe tu mensaje aquí"
                    rows={6}
                    className="w-full px-4 py-3 bg-white/50 dark:bg-black/50 border border-gray-300 dark:border-gray-700 rounded-lg focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition-colors duration-300 resize-none"
                  />
                </div>
